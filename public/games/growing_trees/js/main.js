...
canvas.addEventListener('pointerdown', e => {
  unlockAudio();
  if (mode === 'creator' || creatorActive || mode === 'flow' || flowActive) return;   // the sound creator / flow editor handle their own canvas input
...
canvas.addEventListener('pointermove', e => {
  if (mode === 'creator' || creatorActive || mode === 'flow' || flowActive) return;
...
function loop(now) {
  lastT = now;
  if (mode === 'creator' || creatorActive || mode === 'flow' || flowActive) { requestAnimationFrame(loop); return; }   // the sound creator / flow editor draw their own frames
  try {
