...
7. **Never launch long-lived processes (local server, headless Chrome) directly in the shell tool — it blocks until timeout, and `pkill` hangs too.** Launch detached in a subshell (`(setsid … &)`) and kill by PID (`pgrep` + `kill -9`). Details in `.agents/development.md`.
